...
void LCD_ShowSignedIntNum(int x, int y, int num, unsigned char len, 
                         unsigned int fc, unsigned int bc, unsigned char sizey)
{         
    unsigned char t, temp;
    unsigned char enshow = 0;
    unsigned char sizex = sizey / 2;
    unsigned int abs_num;  // 存储绝对值
    unsigned char is_negative = 0;  // 标记是否为负数

    // 处理负数情况
    if (num < 0) {
        is_negative = 1;
        abs_num = (unsigned int)(-num);  // 取绝对值
    } else {
        is_negative = 0;
    ...
    if (is_negative) {
        ...
        LCD_ShowChar(x, y, ' ', fc, bc, sizey, 0);  // 正数显示空格
    }

    // 显示数字部分（从第二位开始）
    for (t = 1; t < len; t++) {
        // 计算当前位的数字（len-1位数字）
        temp = (abs_num / mypow(10, (len - 1) - t)) % 10;
